...
class PDMScorer:
    ...
    def __init__(
        ...
        self._offroad_time_idcs: Optional[npt.NDArray[np.float64]] = None
        self._collision_positions: Optional[npt.NDArray[np.float64]] = None
        self._collision_actor_positions: Optional[npt.NDArray[np.float64]] = None
        self._collision_agent_positions: Optional[npt.NDArray[np.float64]] = None
        self._collision_is_agent: Optional[npt.NDArray[np.bool_]] = None
        self._offroad_positions: Optional[npt.NDArray[np.float64]] = None
        self._collision_track_tokens: Optional[List[Optional[str]]] = None
    ...
    def export_infraction_features(self) -> Dict[str, Any]:
        """
        Export per-proposal collision/offroad diagnostics.
        Arrays are indexed by proposal mode.
        """
        collision_time_idx = np.asarray(self._collision_time_idcs, dtype=np.float32).copy()
        offroad_time_idx = np.asarray(self._offroad_time_idcs, dtype=np.float32).copy()
        return {
            "collision_time_idx": collision_time_idx,
            "collision_time_s": collision_time_idx * float(self.proposal_sampling.interval_length),
            "collision_position_xy": np.asarray(self._collision_positions, dtype=np.float32).copy(),
            "collision_actor_position_xy": np.asarray(
                self._collision_actor_positions, dtype=np.float32
            ).copy(),
            "collision_agent_position_xy": np.asarray(
                self._collision_agent_positions, dtype=np.float32
            ).copy(),
            "collision_is_agent": np.asarray(self._collision_is_agent, dtype=np.bool_).copy(),
            "collision_valid": np.isfinite(collision_time_idx),
            "collision_track_token": list(self._collision_track_tokens),
            "offroad_time_idx": offroad_time_idx,
            "offroad_time_s": offroad_time_idx * float(self.proposal_sampling.interval_length),
            "offroad_position_xy": np.asarray(self._offroad_positions, dtype=np.float32).copy(),
            "offroad_valid": np.isfinite(offroad_time_idx),
        }
    ...
    def _reset(
        ...
        self._offroad_time_idcs = np.zeros(self._num_proposals, dtype=np.float64)
        self._collision_time_idcs.fill(np.inf)
        self._ttc_time_idcs.fill(np.inf)
        self._offroad_time_idcs.fill(np.inf)
        self._collision_positions = np.full((self._num_proposals, 2), np.nan, dtype=np.float64)
        self._collision_actor_positions = np.full((self._num_proposals, 2), np.nan, dtype=np.float64)
        self._collision_agent_positions = np.full((self._num_proposals, 2), np.nan, dtype=np.float64)
        self._collision_is_agent = np.zeros(self._num_proposals, dtype=np.bool_)
        self._offroad_positions = np.full((self._num_proposals, 2), np.nan, dtype=np.float64)
        self._collision_track_tokens = [None for _ in range(self._num_proposals)]
    ...
    def _calculate_ego_area(self) -> None:
        ...
        # cache first offroad timestamp and ego center position for diagnostics
        offroad_any = batch_nondrivable_area_mask.any(axis=-1)
        if offroad_any.any():
            first_offroad_idx = batch_nondrivable_area_mask.argmax(axis=-1)
            for proposal_idx in np.nonzero(offroad_any)[0]:
                time_idx = int(first_offroad_idx[proposal_idx])
                self._offroad_time_idcs[proposal_idx] = float(time_idx)
                self._offroad_positions[proposal_idx] = self._ego_coords[
                    proposal_idx, time_idx, BBCoordsIndex.CENTER
                ]
        ...
        for time_idx in range(self.proposal_sampling.num_poses + 1):
            ...
            for proposal_idx, geometry_idx in zip(intersecting[0], intersecting[1]):
                ...
                if collisions_at_stopped_track_or_active_front or (
                    ...
                    prev_collision_idx = self._collision_time_idcs[proposal_idx]
                    self._collision_time_idcs[proposal_idx] = min(time_idx, prev_collision_idx)
                    if float(time_idx) <= float(prev_collision_idx):
                        self._collision_positions[proposal_idx] = self._ego_coords[
                            proposal_idx, time_idx, BBCoordsIndex.CENTER
                        ]
                        actor_center = tracked_object.box.center
                        self._collision_actor_positions[proposal_idx] = np.array(
                            [actor_center.x, actor_center.y], dtype=np.float64
                        )
                        is_agent = tracked_object.tracked_object_type in AGENT_TYPES
                        self._collision_is_agent[proposal_idx] = bool(is_agent)
                        if is_agent:
                            self._collision_agent_positions[proposal_idx] = np.array(
                                [actor_center.x, actor_center.y], dtype=np.float64
                            )
                        else:
                            self._collision_agent_positions[proposal_idx] = np.array(
                        ...
                        self._collision_track_tokens[proposal_idx] = token
