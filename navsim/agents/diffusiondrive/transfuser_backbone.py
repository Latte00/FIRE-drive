...
class TransfuserBackbone(nn.Module):
    ...
    def __init__(self, config: TransfuserConfig):
        ...
        super().__init__()
        self.config = config
        # 直接使用本地预训练权重文件，避免网络连接问题
        pretrained_cfg_overlay = None
        use_pretrained = bool(config.bkb_path) and os.path.isfile(config.bkb_path)
        if use_pretrained:
    ...
    def forward(self, image, lidar):
        ...
        return features, fused_features, image_features, lidar_features
